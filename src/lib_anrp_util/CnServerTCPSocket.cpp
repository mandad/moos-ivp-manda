...
CnServerTCPSocket::CnServerTCPSocket(std::string host, int port)
{
	initialize(host, port);
}

void CnServerTCPSocket::initialize(std::string host, int port)
{
	this->port = port;
	this->host = host;
	int yes = 1;
	...
	if(listen_fd == -1) throw std::runtime_error(ssp("error with socket: %s", strerror(errno)));
	...
	if(bind(listen_fd, (struct sockaddr *)&madr, sizeof(struct sockaddr)) == -1) {
		throw std::runtime_error(ssp("error binding: %s", strerror(errno)));
	}
	
	if(listen(listen_fd, 5) == -1) {
		throw std::runtime_error(ssp("error listening: %s", strerror(errno)));
	}
}
...
CnFDCtl *CnServerTCPSocket::ReceiveConnectionDirect()
{
	...
	if(newfd == -1) {
		throw std::runtime_error(ssp("error accepting: %s", strerror(errno)));
	}
